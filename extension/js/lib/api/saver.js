...
class KorobkaSaver {
        ...
        async save(msg) {
                try {
                        ...
                        if (app) {
                                try {
                                        const data = await this.#apiAddItem(image, msg.folderID, msg);
         ...
         * первым), затем base64. meta — {title?, annotation?, tags?} из окна
         * сохранения. Возвращает data успешного ответа.
         */
        async #apiAddItem(image, folderIdRaw, meta = {}) {
                ...
                // Метаданные элемента: название, комментарий, теги (v0.6).
                const metaBody = {};
                const annotation = String(meta.annotation || "").trim().slice(0, 2000);
                if (annotation) metaBody.annotation = annotation;
                const title = String(meta.title || "").trim().slice(0, 200);
                if (title) metaBody.title = title;
                if (Array.isArray(meta.tags)) {
                        const tags = meta.tags.map((t) => String(t).trim()).filter(Boolean).slice(0, 30);
                        if (tags.length) metaBody.tags = tags;
                }
                        ...
                        try {
                                const resp = await this.#fetchWithTimeout(
                                        `http://127.0.0.1:${this.#activePort}/api/item/add`,
                                        {
                                                ...
                                                body: JSON.stringify({ url, folderId, ...metaBody }),
                                        },
                                        60000,
                                );
                                const json = await resp.json().catch(() => null);
                ...
                if (image.base64) {
                        const resp = await this.#fetchWithTimeout(
                                `http://127.0.0.1:${this.#activePort}/api/item/add`,
                                {
                                        method: "POST",
                                        headers: { "Content-Type": "application/json" },
                                        body: JSON.stringify({ base64: image.base64, mime: image.mime || "", filename: image.filename || "", folderId, ...metaBody }),
                                },
                                60000,
                        );
                        const json = await resp.json().catch(() => null);
